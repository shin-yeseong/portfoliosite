...
function SkillCard({ icon, label }) {
  return (
    <motion.div
      ...
      className="flex flex-col items-center justify-center p-6 bg-white/60 dark:bg-darkSecondaryBg/60 rounded-2xl border border-gray-100 dark:border-slate-700 shadow-lg hover:shadow-xl transition-shadow duration-300 group w-40 h-40"
...
export default function Skills() {
  return (
    ...
    >
      ...
      />
      <motion.h2 variants={itemVariants} className="text-5xl font-extrabold text-gray-900 dark:text-white mb-16 tracking-tight z-10">Skills</motion.h2>
      <motion.div variants={itemVariants} className="flex flex-col gap-12 w-full max-w-5xl z-10 px-4 max-h-full overflow-y-auto fp-auto-scroll">
        {categories.map((cat) => (
          <div key={cat.name}>
            <motion.h3
              variants={itemVariants}
              className="text-2xl font-bold text-darkBg dark:text-lightBg mb-6 pl-2"
            >
              {cat.name}
            </motion.h3>
