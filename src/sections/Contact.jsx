...
export default function Contact() {
  return (
    ...
    >
      ...
      />
      <motion.h2 variants={itemVariants} className="text-3xl sm:text-4xl md:text-5xl font-extrabold text-darkBg dark:text-lightBg mb-8 md:mb-12 tracking-tight z-10">Contact</motion.h2>
      <motion.div variants={containerVariants} className="flex flex-col gap-6 max-w-md w-full z-10">
        {contacts.map((c) => (
          <motion.a
            ...
            className="flex items-center gap-4 px-4 py-3 rounded-2xl bg-lightSecondaryBg dark:bg-darkSecondaryBg border border-lightSecondaryBg dark:border-darkSecondaryBg shadow-none hover:shadow-lg transition text-sm sm:text-base md:text-lg font-semibold text-darkBg dark:text-lightBg hover:text-accentBlue dark:hover:text-accentBlue"
          >
            <span className="text-xl sm:text-2xl md:text-2xl text-darkSecondaryBg dark:text-lightSecondaryBg">{c.icon}</span>
            <span>{c.label}</span>
            <span className="ml-auto text-xs sm:text-sm md:text-base text-darkSecondaryBg dark:text-lightSecondaryBg font-normal">{c.value}</span>
          </motion.a>
        ))}
      </motion.div>
